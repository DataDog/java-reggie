...
public record LinearTemplatePlan(List<Op> ops, int groupCount) {
  ...
  public record Op(OpKind kind, int groupNumber, String literal, char delimiter) {
    static Op literal(String literal) {
      return new Op(OpKind.LITERAL, 0, literal, (char) 0);
    }

    static Op capture(OpKind kind, int groupNumber) {
      return new Op(kind, groupNumber, null, (char) 0);
    }

    static Op captureUntil(OpKind kind, int groupNumber, char delimiter) {
      return new Op(kind, groupNumber, null, delimiter);
    }

    static Op uncaptured(OpKind kind) {
      return new Op(kind, 0, null, (char) 0);
    }
  }
  ...
  private static boolean isQuotedCapture(List<PatternAtom> atoms, int index) {
    ...
    if (atom.kind() != PatternAtom.Kind.UNTIL_DELIMITER || atom.delimiter() != '"') return false;
    return index > 0
        && index + 1 < atoms.size()
