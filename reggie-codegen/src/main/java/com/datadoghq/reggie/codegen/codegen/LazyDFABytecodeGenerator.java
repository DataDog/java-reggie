...
public class LazyDFABytecodeGenerator {
  ...
  /**
   * Mirrors {@code LazyDFACache.NEEDS_INT_ARRAY_ACQUIRE}. On x86/TSO the hot loop emits a plain
   * {@code IALOAD} for the ASCII table element read; on weakly-ordered platforms it emits {@code
   * INT_ARRAY_VH.getAcquire} to pair with the release write in {@code cacheEntry}. Computed once at
   * class-load time so the generated bytecode is fixed per JVM invocation.
   */
  private static final boolean NEEDS_INT_ARRAY_ACQUIRE;
   ...
   * directly to the JIT for better optimization. The ASCII table element read uses {@code
   * INT_ARRAY_VH.getAcquire} on weakly-ordered platforms ({@link #NEEDS_INT_ARRAY_ACQUIRE}) and a
   * plain {@code IALOAD} on x86/TSO where TSO provides acquire semantics for free.
   *
   * <p>Equivalent Java:
   *
   * <pre>
  ...
  public void generateMatchesMethod(ClassWriter cw, String className) {
    ...
    // int next = (table != null && c < 128) ? <read table[c]> : UNCACHED  (slot 8)
    // On weakly-ordered platforms (ARM/RISC-V): INT_ARRAY_VH.getAcquire pairs with the
    // setRelease in LazyDFACache.cacheEntry, making nfaStateSets/accepting visible.
    // On x86/TSO: TSO gives loads acquire semantics for free — plain IALOAD is sufficient.
    ...
    if (NEEDS_INT_ARRAY_ACQUIRE) {
      // ARM/RISC-V: acquire read via VarHandle
      ...
      // x86/TSO: plain array element load — zero VarHandle overhead
      mv.visitVarInsn(ALOAD, 7); // table
      mv.visitVarInsn(ILOAD, 6); // c
      mv.visitInsn(IALOAD);
    }
