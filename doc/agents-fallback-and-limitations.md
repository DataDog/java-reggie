...
- **`ONEPASS_NFA` `findMatch()`/`findBoundsFrom()` regression**: boolean `find()` on `ONEPASS_NFA`
  patterns is native and fast (~3.5x JDK, fixed in `6841723`). The group-capturing find path
  (`findMatch()`, `findBoundsFrom()`, generated by `OnePassBytecodeGenerator.generateFindMatchFromMethod`,
  lines 1088-1224) still re-scans candidate match lengths via `matchesInRange` after `findFrom`
  locates the match start, instead of reusing the end offset `matchFrom` already computed —
  measured ~1.6-1.7x slower than JDK on long haystacks and growing with input length.
