...
**Runtime-vs-compile-time inconsistency (RESOLVED):** the annotation processor now rejects every
FULL_FALLBACK strategy at build time. `ReggieMatcherBytecodeGenerator.generate()` throws
`UnsupportedOperationException` when `StrategyJdkClassifier.classifyJdkDependency(strategy) ==
FULL_FALLBACK` (alongside the existing AST-level rejections for `anchorConditionDiluted`,
`alternationPriorityConflict`, and `FallbackPatternDetector.needsFallback`), and
`RegexPatternProcessor` turns that into a build `Diagnostic.Kind.ERROR` on the annotated element. A
fixed `@RegexPattern` class cannot fall back to `java.util.regex` at runtime the way
`Reggie.compile()` does, so emitting the (known-incorrect) native bytecode is no longer possible —
the build fails with a message pointing the user to `Reggie.compile()`. The five FULL_FALLBACK
strategies are: `SPECIALIZED_MULTIPLE_LOOKAHEADS`, `SPECIALIZED_LITERAL_LOOKAHEADS`,
`HYBRID_DFA_LOOKAHEAD`, `VARIABLE_CAPTURE_BACKREF`, `NESTED_QUANTIFIED_GROUPS`. The benchmark
`@RegexPattern` fixtures that resolved to these were converted to obtain their matchers via
`Reggie.compile()` (the correct runtime path), preserving each benchmark's pattern and intent.

RICH_API_HYBRID is **not** rejected: those strategies (`SPECIALIZED_LITERAL_ALTERNATION`,
`FIXED_REPETITION_BACKREF`) are fully correct at compile time (native correct booleans + JDK-correct
group extraction via the base defaults), so they still generate and only emit a `MANDATORY_WARNING`.
