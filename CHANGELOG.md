...
- fix: review findings from the 0.4.0 arc — counted-loop route refuses lookaround/conditional/atomic combos (the backtracker treated assertions as epsilon: false matches), the annotation processor refuses counted-loop-lowered patterns (only the runtime pipeline executes the markers), nested counted loops reset their counters per outer iteration, hybrid patterns return a fresh matcher per compile() (the nfa-half is mutable; L1 sharing was a data race), (?i) with (?u)/(?U) refuses (the JDK folds the special Unicode cases there; reggie's simple folding diverged), the JIT size gate applies on structural-cache hits, the classfile size scanner reads the full long/double payload, missing prefilter facts are negative-cached, clearCache clears the new caches, and the rust benchmark engine's close() is idempotent
- perf: 1-char prefilter facts scan via the char indexOf intrinsic — the String overload's first-char scan runs ~5.7x slower on x86 (2.73 vs 0.48us over a 22KB no-match input); IAST LdapNoMatch LONG 276 -> 2,026 ops/ms (7.3x, ahead of re2j 358 / jdk 90 / rust 17), real-corpus no-match -1.7% raw / ~9% vs the same-run jdk control; ci 1-char facts take a dual lower/upper SIMD scan
