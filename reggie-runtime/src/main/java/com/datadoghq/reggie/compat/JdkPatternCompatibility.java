 ...
 *
 * The engines fold case differently for <em>non-ASCII pattern letters</em>: the JDK (without {@code
 * UNICODE_CASE}) folds only ASCII {@code a-z}/{@code A-Z}, while Reggie's {@code (?i)} also folds
 * Unicode letters ({@code "é"} becomes {@code [éÉ]}). Input-side non-ASCII characters do
 ...
 *       ASCII (where both engines fold exactly the ASCII alphabet) and is the recommended migration
 *       entry point.
