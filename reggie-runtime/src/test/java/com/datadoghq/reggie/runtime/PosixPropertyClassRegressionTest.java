 ...
 *
 * <p>History: {@code \p{Alnum}}, {@code \p{Alpha}}, {@code \p{ASCII}}, {@code \p{Cntrl}}, {@code
 * \p{Lower}}, {@code \p{Upper}} and {@code \p{IsAlphabetic}} threw "Unsupported Unicode property" —
 * 7 hard-rejected consumer sites in logs-backend. The JDK forms are ASCII-only unless {@code
 * UNICODE_CHARACTER_CLASS} is set (which Reggie does not support), so they map onto the existing
 * ASCII CharSets; the {@code IsXxx} forms stay Unicode-aware.
 *
...
class PosixPropertyClassRegressionTest {
  ...
  private static final int[] PROBES = {
    0, 1, 7, 8, 9, 10, 11, 12, 13, 31, 32, 33, 45, 47, 48, 57, 58, 64, 65, 70, 90, 91, 95, 96, 97,
    102, 127, 128, 160, 233, 255, 945, 940, 917, 19968, 65279, 8364
  };
  ...
  void isAlphabeticMatchesJdk() {
    ...
    for (int c : PROBES) {
      ...
      assertEquals(
          jp.matcher(s).matches(),
          rm.matches(s),
          "IsAlphabetic diverged for U+" + Integer.toHexString(c));
    }
    // The '.*[\\p{IsAlphabetic}].*' shape previously exceeded the 64KB generated-method
    // limit and was rejected on strict compile; since the transition-merge + lookup-table
    // codegen fix it compiles NATIVELY and must match the JDK directly (the fallback path
    // remains JDK-equivalent for any shape that still declines).
    var native_ = Reggie.compile(".*[\\p{IsAlphabetic}].*");
    var jp2 = Pattern.compile(".*[\\p{IsAlphabetic}].*");
    assertEquals(jp2.matcher("ab\u00e9").matches(), native_.matches("ab\u00e9"));
    assertEquals(jp2.matcher("123").matches(), native_.matches("123"));
    assertEquals(jp2.matcher("\u4e2d\u6587x").matches(), native_.matches("\u4e2d\u6587x"));
  }
