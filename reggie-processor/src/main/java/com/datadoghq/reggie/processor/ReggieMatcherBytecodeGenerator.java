...
public class ReggieMatcherBytecodeGenerator {
  ...
  public Realization resolveRealization(boolean allowJdkFallback) throws Exception {
    ...
    // BITSTATE_CAPTURE is never emitted by the annotation-processor path: PatternAnalyzer only
    // substitutes it for the dynamic RuntimeCompiler.compile() path, but a pattern that is
    // BITSTATE_CAPTURE-eligible is by definition also PIKEVM_CAPTURE-eligible (same NFA-interpreter
    // semantics), so it is folded into the same branch here and realized as DELEGATE_PIKEVM —
    // meaning a @RegexPattern-compiled BITSTATE_CAPTURE-eligible pattern still runs on
    // PikeVMMatcher
    // at runtime, while the same pattern compiled via RuntimeCompiler.compile() gets
    // BitStateMatcher.
    // That asymmetry is intentional for P1.
  ...
  public byte[] generate() throws Exception {
    ...
    if (strategy == PatternAnalyzer.MatchingStrategy.PIKEVM_CAPTURE
      ...
      // PIKEVM_CAPTURE (and its BITSTATE_CAPTURE substitution, see resolveRealization) requires a
      // lazy-aware NFA for correct shortest-match semantics. Bytecode generation is not supported
      // at annotation-processing time; route to runtime.
    ...
    return nameMap.isEmpty() ? bytes : injectNamePropagation(bytes);
  }

  /**
   * Post-processes generated bytecode to replace every 4-arg {@code MatchResultImpl} constructor
   * call with the 5-arg variant that also receives {@code this.nameToIndex}, so named-group
   * accessors work on annotation-processor-generated matchers.
   */
  private byte[] injectNamePropagation(byte[] classBytes) {
        ...
        new ClassVisitor(ASM9, cw) {
          ...
          public MethodVisitor visitMethod(
            ...
            return new MethodVisitor(
                ASM9, super.visitMethod(access, name, descriptor, signature, exceptions)) {
              @Override
              public void visitMethodInsn(
                  int opcode, String owner, String mname, String mdescriptor, boolean isInterface) {
                if (opcode == INVOKESPECIAL
                    && "com/datadoghq/reggie/runtime/MatchResultImpl".equals(owner)
                    && "<init>".equals(mname)
                    && "(Ljava/lang/String;[I[II)V".equals(mdescriptor)) {
                  ...
                  super.visitMethodInsn(
                      ...
                      owner,
                      mname,
                      "(Ljava/lang/String;[I[IILjava/util/Map;)V",
                      isInterface);
                } else {
